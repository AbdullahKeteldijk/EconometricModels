...
%% 4. Initial Parameter Values
      ...
      theta_ini = [alpha_ini,beta_ini,sig_ini,beta2_ini,beta3_ini];
      
      
%% 5. Parameter Space Bounds
        
      lb=[-1000,-1000,0.00001,-1000,-1000];  % lower bound for theta
      ub=[1000,1000,1000,1000,1000];        % upper bound for theta
...
%% 6. Optimize Log Likelihood Criterion
      ...
      [theta_hat,llik_val,exitflag]=...
          fmincon(@(theta) - llik_fun_AR3(x,theta),theta_ini,[],[],[],[],lb,ub,[],options);
...
xf(3) = x(end-2);
xr = data(456:end,1);
    for t=4:T % start recursion from t=2 to t=T
        
       xf(t) =  alpha + beta * xf(t-1) + beta2 * xf(t-2) + beta3 * xf(t-3) + epsilon(t); % generate x(t) recursively
